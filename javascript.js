//引入百度地图
document.write("<script language=javascript src='http://api.map.baidu.com/api?v=2.0&ak=RoZhhckPEAigbGNvZt7Wnxz4rZENzBzL'></script>");

//引入eachar
document.write("<script language=javascript src='http://echarts.baidu.com/gallery/vendors/echarts/echarts.min.js'></script>");
document.write("<script language=javascript src='http://echarts.baidu.com/gallery/vendors/echarts-gl/echarts-gl.min.js'></script>");
document.write("<script language=javascript src='http://echarts.baidu.com/gallery/vendors/echarts-stat/ecStat.min.js'></script>");
document.write("<script language=javascript src='http://echarts.baidu.com/gallery/vendors/echarts/extension/dataTool.min.js'></script>");
document.write("<script language=javascript src='http://echarts.baidu.com/gallery/vendors/echarts/map/js/china.js'></script>");
document.write("<script language=javascript src='http://echarts.baidu.com/gallery/vendors/echarts/map/js/world.js'></script>");
document.write("<script language=javascript src='http://echarts.baidu.com/gallery/vendors/echarts/extension/bmap.min.js'></script>");
document.write("<script language=javascript src='http://echarts.baidu.com/gallery/vendors/simplex.js'></script>");
document.write("<script src='https://cdn.staticfile.org/jquery/1.10.2/jquery.min.js'></script>");
document.write("<script type='text/javascript' src='http://api.map.baidu.com/library/Heatmap/2.0/src/Heatmap_min.js'></script>");
 
//引入jquire
//document.write(" <script type='text/javascript' src='http://cj.cbcie.com/js/jquery-1.6.min.js'></script>");



//显示百度地图
function showMap(divId){
    // 百度地图API功能
	var map = new BMap.Map(divId);    // 创建Map实例
	map.centerAndZoom(new BMap.Point(121.45, 31.30), 9);  // 初始化地图,设置中心点坐标和地图级别
	//添加地图类型控件
	map.addControl(new BMap.MapTypeControl({
		mapTypes:[
            BMAP_NORMAL_MAP,
            BMAP_HYBRID_MAP
        ]}));	  
	map.setCurrentCity("上海");          // 设置地图显示的城市 此项是必须设置的
	map.enableScrollWheelZoom(true);     //开启鼠标滚轮缩放
}


//显示车流量图(1时用自己的数据，2时用百度的数据demo)
function showTaxiFlow1(divId,jsonUrl){

    var dom = document.getElementById(divId);
    var myChart = echarts.init(dom);
    var app = {};
	var pois = new Array();//BMap.Point类型的坐标点
    var pois_overspeed = new Array();//超速的坐标点，BMap.Point类型
    var string_attr = [];//属性值
    option = null;
    app.title = '上海出租路线 - 线特效';

//坐标必须是转换后的坐标（BD09）
//偏移距离始终代表对应经纬度的最后几位
$.get(jsonUrl, function(data1){
   //$.get()是juquery用法，function中的data是返回的数据
    data1=JSON.parse(data1);  //将返回的字符串转为对象
    var hStep = 300 / (data1.length - 1);

    
    //var busLines
    var busLines = [].concat.apply([], data1.map(function (busLine, idx) {
        var prevPt;
        var points = [];
		 //添加属性标注内容
        if(busLine[2] == 0){
          string_attr += "业务状态：正常" + "<br />";
        }
        else{
          string_attr += "业务状态：报警" + "<br />";
        }
        if(busLine[3] == 0){
          string_attr += "载客状态：载人" + "<br />";
        }
        else{
          string_attr += "载客状态：空车" + "<br />";
        }
        if(busLine[4] == 0){
          string_attr += "顶灯状态：营运" + "<br />";
        }
        else if(busLine[4] == 1){
          string_attr += "顶灯状态：待运" + "<br />";
        }
        else if(busLine[4] == 2){
          string_attr += "顶灯状态：电调" + "<br />";
        }
        else if(busLine[4] == 3){
          string_attr += "顶灯状态：暂停" + "<br />";
        }
        else if(busLine[4] == 4){
          string_attr += "顶灯状态：求助" + "<br />";
        }
        else if(busLine[4] == 5){
          string_attr += "顶灯状态：停运" + "<br />";
        }
        if(busLine[5] == 0){
          string_attr +="道路状态：地面道路" + "<br />";
        }
        else{
          string_attr +="道路状态：快速道路";
        }

        for (var i = 0; i < busLine.length; i += 7) 
        {
            var pt = [busLine[i], busLine[i + 1]];
            points.push([pt[0] / 1e6, pt[1] / 1e6]);
			 pois.push(new BMap.Point(pt[0] / 1e6, pt[1] / 1e6 ));

            if(busLine[i+6] > 40){
              pois_overspeed.push(new BMap.Point(pt[0] / 1e6, pt[1] / 1e6 ));
            }
        }
	
        return{
            coords: points,
            lineStyle: {
                normal: {
                    color: echarts.color.modifyHSL('#FFFF00', Math.round(hStep * idx))
                }
            }
        };
    })); //end of var busLines

    myChart.clear();
    myChart.setOption(option = {
        bmap: {
            center: [121, 31],
            zoom: 10,
            roam: true,
            mapStyle: {
              'styleJson': [
                
              ]
            }
        },
        series: [{
            type: 'lines',
            coordinateSystem: 'bmap',
            polyline: true,
            data: busLines,
            silent: true,
            lineStyle: {
                normal: {
                    // color: '#c23531',
                    // color: 'rgb(200, 35, 45)',
                    opacity: 0.2,
                    width: 10
                }
            },
            progressiveThreshold: 500,
            progressive: 200
        }, {
            type: 'lines',
            coordinateSystem: 'bmap',
            polyline: true,
            data: busLines,
            lineStyle: {
                normal: {
                    width: 0
                }
            },
            effect: {
                constantSpeed: 100,
                show: true,
                trailLength: 0.1,
                symbolSize: 1.5
            },
            zlevel: 1
        }]
    });  //end of  myChart.setOption

        //使用bmap实例引用百度地图控件
    if (!app.inNode) {
      // 添加百度地图插件
      var bmap = myChart.getModel().getComponent('bmap').getBMap();
      var top_left_control = new BMap.ScaleControl({anchor: BMAP_ANCHOR_TOP_LEFT});// 左上角，添加比例尺
	    var top_left_navigation = new BMap.NavigationControl();  //左上角，添加默认缩放平移控件
		

      bmap.addControl(new BMap.MapTypeControl());
      bmap.addControl(top_left_control);
      bmap.addControl(top_left_navigation);
	  
	  var polyline = drawPolyline(bmap,pois,string_attr);
      var marker = setMarker(bmap,polyline,string_attr);
      addPointOverspeed(bmap,pois_overspeed)
    }

});;//end of get


}


function showTaxiFlow2(divId,jsonUrl,ifRefresh){

   //此处不完美，应该只刷新div,但是无法解决echart更新问题
    if(ifRefresh==1)  //当点击取消的时候刷新整个页面，否则echart显示有问题
    {
      window.location.reload();
    }
   
    var dom = document.getElementById(divId);
    var myChart = echarts.init(dom);
    var app = {};
    option = null;
    app.title = '北京公交路线 - 线特效';

//坐标必须是转换后的坐标（BD09）
//偏移距离始终代表对应经纬度的最后几位
$.get(jsonUrl, function(data2){
   //$.get()是juquery用法，function中的data是返回的数据
    //data1=JSON.parse(data1);
    var hStep = 300 / (data2.length - 1);
    

    //var busLines
    var busLines = [].concat.apply([], data2.map(function (busLine, idx) {
        var prevPt;
        var points = [];

        for (var i = 0; i < busLine.length; i += 2) 
        {
             var pt = [busLine[i], busLine[i + 1]];
            if (i > 0) 
            {
                pt = [
                    prevPt[0] + pt[0],
                    prevPt[1] + pt[1]
                ];
             }

            prevPt = pt;

            points.push([pt[0] / 1e6, pt[1] / 1e6]);
			
        }
		
        return{
            coords: points,
            lineStyle: {
                normal: {
                    color: echarts.color.modifyHSL('#5A94DF', Math.round(hStep * idx))
                }
            }
        };
    })); //end of var busLines

    myChart.setOption(option = {
        bmap: {
            center: [121.5, 31.2],
            zoom: 11,
            roam: true,
            mapStyle: {
              'styleJson': [
               {
                  'featureType': 'water',
                  'elementType': 'all',
                  'stylers': {
                    'color': '#031628'
                  }
                },
                {
                  'featureType': 'land',
                  'elementType': 'geometry',
                  'stylers': {
                    'color': '#000102'
                  }
                },
                {
                  'featureType': 'highway',
                  'elementType': 'all',
                  'stylers': {
                    'visibility': 'off'
                  }
                },
                {
                  'featureType': 'arterial',
                  'elementType': 'geometry.fill',
                  'stylers': {
                    'color': '#000000'
                  }
                },
                {
                  'featureType': 'arterial',
                  'elementType': 'geometry.stroke',
                  'stylers': {
                    'color': '#0b3d51'
                  }
                },
                {
                  'featureType': 'local',
                  'elementType': 'geometry',
                  'stylers': {
                    'color': '#000000'
                  }
                },
                {
                  'featureType': 'railway',
                  'elementType': 'geometry.fill',
                  'stylers': {
                    'color': '#000000'
                  }
                },
                {
                  'featureType': 'railway',
                  'elementType': 'geometry.stroke',
                  'stylers': {
                    'color': '#08304b'
                  }
                },
                {
                  'featureType': 'subway',
                  'elementType': 'geometry',
                  'stylers': {
                    'lightness': -70
                  }
                },
                {
                  'featureType': 'building',
                  'elementType': 'geometry.fill',
                  'stylers': {
                    'color': '#000000'
                  }
                },
                {
                  'featureType': 'all',
                  'elementType': 'labels.text.fill',
                  'stylers': {
                    'color': '#857f7f'
                  }
                },
                {
                  'featureType': 'all',
                  'elementType': 'labels.text.stroke',
                  'stylers': {
                    'color': '#000000'
                  }
                },
                {
                  'featureType': 'building',
                  'elementType': 'geometry',
                  'stylers': {
                    'color': '#022338'
                  }
                },
                {
                  'featureType': 'green',
                  'elementType': 'geometry',
                  'stylers': {
                    'color': '#062032'
                  }
                },
                {
                  'featureType': 'boundary',
                  'elementType': 'all',
                  'stylers': {
                    'color': '#465b6c'
                  }
                },
                {
                  'featureType': 'manmade',
                  'elementType': 'all',
                  'stylers': {
                    'color': '#022338'
                  }
                },
                {
                  'featureType': 'label',
                  'elementType': 'all',
                  'stylers': {
                    'visibility': 'off'
                  }
                }
              ]
            }
        },
        series: [{
            type: 'lines',
            coordinateSystem: 'bmap',
            polyline: true,
            data: busLines,
            silent: true,
            lineStyle: {
                normal: {
                    // color: '#c23531',
                    // color: 'rgb(200, 35, 45)',
                    opacity: 0.2,
                    width: 1
                }
            },
            progressiveThreshold: 500,
            progressive: 200
        }, {
            type: 'lines',
            coordinateSystem: 'bmap',
            polyline: true,
            data: busLines,
            lineStyle: {
                normal: {
                    width: 0
                }
            },
            effect: {
                constantSpeed: 20,
                show: true,
                trailLength: 0.1,
                symbolSize: 1.5
            },
            zlevel: 1
        }]
    });  //end of  myChart.setOption


    //使用bmap实例引用百度地图控件
    if (!app.inNode) {
      // 添加百度地图插件
      var bmap = myChart.getModel().getComponent('bmap').getBMap();

      var top_left_control = new BMap.ScaleControl({anchor: BMAP_ANCHOR_TOP_LEFT});// 左上角，添加比例尺
	    var top_left_navigation = new BMap.NavigationControl();  //左上角，添加默认缩放平移控件
		 
	        
	       
      bmap.addControl(new BMap.MapTypeControl());
      bmap.addControl(top_left_control);
      bmap.addControl(top_left_navigation);
  }

});;//end of get

 


}


//侧边栏消失与显示
function sidebarAppearance(divId){
  var sidebar=document.getElementById(divId);
  if(sidebar.style.display=="none"){
    sidebar.style.display="";
  }
  else{
    sidebar.style.display="none";
  }

/*
  if(sidebar.style.display==""){
    sidebar.style.display="none";
  }*/
}

function showinfo(jsonUrl){
	option = null;
	
$.get(jsonUrl, function(data1){
	data1=JSON.parse(data1);
	var lert=data1[0][2];
	var isempty=data1[0][3];
	var lamp=data1[0][4];
	var road=data1[0][5];	
   	if(lert==0){
        document.getElementById("lert").innerHTML="业务状态：正常";
	}
	if(lert==1){
		document.getElementById("lert").innerHTML="业务状态：报警";
	}
	
	if(isempty==0){
        document.getElementById("isempty").innerHTML="载客状态：载人";
	}
	if(isempty==1){
	    document.getElementById("isempty").innerHTML="载客状态：空车";
	}
	
	if(lamp==0){
        document.getElementById("lamp").innerHTML="顶灯状态：营运";
	}
	if(lamp==1){
		document.getElementById("lamp").innerHTML="顶灯状态：待运";
	}
	if(lamp==2){
		document.getElementById("lamp").innerHTML="顶灯状态：电调";
	}
	if(lamp==3){
		document.getElementById("lamp").innerHTML="顶灯状态：暂停";
	}
	if(lamp==4){
		document.getElementById("lamp").innerHTML="顶灯状态：求助";
	}
	if(lamp==5){
		document.getElementById("lamp").innerHTML="顶灯状态：停运";
	}
	
	if(road==0){
        document.getElementById("road").innerHTML="道路状态：地面道路";
	}
	if(road==1){
	    document.getElementById("road").innerHTML="道路状态：快速道路";
	}
 });
	
    
}

function showCircle(divId,jsonUrl){
	option = null;
	var dom = document.getElementById(divId);
    var myChart = echarts.init(dom);
	//document.getElementById("r-result").innerHTML="空"
	$.get(jsonUrl, function(data1){
		data1=JSON.parse(data1);
		  var busLines = [].concat.apply([], data1.map(function (busLine, idx) {
			  var pt1=[busLine[0]/1e6,busLine[1]/1e6];
		      var i=busLine.length/7-1;
		      var pt2=[busLine[i*7]/1e6,busLine[i*7+1]/1e6];
			  var bmap = myChart.getModel().getComponent('bmap').getBMap();
		var mPoint1 = new BMap.Point(pt1[0], pt1[1]);  
        var mPoint2 = new BMap.Point(pt2[0], pt2[1]); 		
	    var circle1 = new BMap.Circle(mPoint1,100,{fillColor:"blue", strokeWeight: 1 ,fillOpacity: 0.3, strokeOpacity: 0.3});
		var circle2 = new BMap.Circle(mPoint2,100,{fillColor:"blue", strokeWeight: 1 ,fillOpacity: 0.3, strokeOpacity: 0.3});
        
		
        var local1 =  new BMap.LocalSearch(bmap, {renderOptions: {map: bmap, autoViewport: false},onSearchComplete: function(results){
			// 判断状态是否正确
			if (local1.getStatus() == BMAP_STATUS_SUCCESS){
				var s = ["起点目标信息:\n"];
				for (var i = 0; i < results.getCurrentNumPois(); i ++){
					s.push(results.getPoi(i).title);
				}
				document.getElementById("r-result1").innerHTML = s.join("<br/>");
			}
		}}); 
		var local2 =  new BMap.LocalSearch(bmap, {renderOptions: {map: bmap, autoViewport: false},onSearchComplete: function(results){
			// 判断状态是否正确
			if (local2.getStatus() == BMAP_STATUS_SUCCESS){
				var s = ["终点目标信息:\n"];
				for (var i = 0; i < results.getCurrentNumPois(); i ++){
					s.push(results.getPoi(i).title);
				}
				document.getElementById("r-result2").innerHTML = s.join("<br/>");
			}
		}}); 
        var input1=document.getElementById("input1");
		var input2=document.getElementById("input2");
		if(input1.value!=""){
		bmap.addOverlay(circle1);
		}
		if(input2.value!=""){
		bmap.addOverlay(circle2);
		}
        local1.searchNearby(input1.value.toString(),mPoint1,100);
		local2.searchNearby(input2.value.toString(),mPoint2,100);
	
		
		
		 }));
		
		
		
	});
}
//当点击查询一辆车的轨迹时
function clickQueryTaxi(divId,startTimeId,endTimeId){
  var input=document.getElementById("input");
  var number=input.value;
  var timeStart=document.getElementById(startTimeId).value;
    var timeEnd=document.getElementById(endTimeId).value;
  params="type=queryTaxi"+"&num="+number+"&startTime="+timeStart+"&endTime="+timeEnd;  //向后台传递参数 //向后台传递参数
  showTaxiFlow1(divId,"http://127.0.0.1:3000/?"+params);
  showinfo("http://127.0.0.1:3000/?"+params);
  showCircle(divId,"http://127.0.0.1:3000/?"+params);
}

function setOverlay(map,pois,string){
  drawPolyline(map,pois,string);
}

function drawPolyline(map,pois,string){ 





  var click = 0;
  var isMarkerOn = 0;
  var polyline = new BMap.Polyline(pois,
  {
          enableEditing: false,//是否启用线编辑
          strokeWeight: '5',
          strokeOpacity: 0,
          strokeColor:"#FFFF00",
          //strokeStyle:'solid'
        })

  map.addOverlay(polyline);
  var cen_p = pois[0];
  map.centerAndZoom(cen_p,15);

  var marker = setMarker(map,polyline,string);
  map.removeOverlay(marker);

  var label = new BMap.Label(string,{
          offset:new BMap.Size(-110,-65)
  });
  marker.addEventListener("click",function(e){
    if(click == 0){
      marker.setLabel(label);
      map.addOverlay(marker);
      click = 1;
    }
    else if(click == 1){
      map.removeOverlay(marker);
      click = 0;
    }
    // else{
    //   map.removeOverlay(marker);
    //   click = 0;
    // }
  })

  polyline.addEventListener("mouseover",function(e){
    // if(isMarkerOn == 0){
    //   map.addOverlay(marker);
    //   isMarkerOn = 1;
    // }
    // else if(isMarkerOn == 1){
    //   map.addOverlay(marker);
    //   isMarkerOn = 0;
    // }
    // marker.setLabel(label);
    // map.addOverlay(marker);
    map.addOverlay(marker);

  })

  // polyline.addEventListener("mouseout",function(e){
  //   marker.setLabel(label);
  //   map.removeOverlay(marker);
  // })

  return polyline;
}

function setMarker(map,polyline,string){


  var click = 0;
  var isShowed = 0;
  var marker = new BMap.Marker(polyline.getPath()[0]);
        // marker.closeInfoWindow() = true;

  map.addOverlay(marker);

  var label = new BMap.Label(string,{
          offset:new BMap.Size(-110,-65)
  });
  marker.setLabel(label);
  // marker.enableDragging();
  // marker.addEventListener("dragend",function(e){
  //   alert("当前位置: ",e.point.lng,", ",e.point.lat);
  // })

  marker.addEventListener("click",function(e){
    map.removeOverlay(marker);
  })
  return marker;
} 

function addPointOverspeed(map,pois){
  var options = {
    size: BMAP_POINT_SIZE_NORMAL,
    shape: BMAP_POINT_SHAPE_STAR,
    color:' #EE4000'
  }

  var pois_set = new BMap.PointCollection(pois,options);
  map.addOverlay(pois_set);
}

function showLampStatus(divId,startTimeId,endTimeId)
{
   
    var timeStart=document.getElementById(startTimeId).value;
    var timeEnd=document.getElementById(endTimeId).value;
    
    var dom = document.getElementById(divId);
    var myChart = echarts.init(dom);

    var img = {
        "0":'data:image/png;base64,',
        '1':'data:image/png;base64,',
        '2':'data:image/png;base64,',
        '3':'data:image/png;base64,',
        '4':'data:image/png;base64,',
        '5':'data:image/png;base64,'
      //'6':'data:image/png;base64,',
      //'7':'data:image/png;base64,',
      //'8':'data:image/png;base64,'
  };
    var array1 = ["营运","待运","电调","暂停","求助","停运"];
    var array = array1.reverse();   //状态名称

    url="type=lampStatus"+"&startTime="+timeStart+"&endTime="+timeEnd;
      $.get("http://127.0.0.1:3000/?"+url,function(data){

        data=JSON.parse(data);

        option = {
          backgroundColor:"rgba(2,13,34,1)",
          tooltip: {
              show:true,
              trigger: 'axis',
              axisPointer: {
                  type: 'line'
              }
          },
          grid: {
              top: '2%',
              bottom: '1%',
              left:'20%',
              containLabel: true
          },
          xAxis: [
              {
                  type: 'value',
                  axisLabel: {
                      show: false,
                      interval: 1,
                      textStyle: {
                          color: '#fff',
                          fontSize: 12
                      }
                  },
                  axisLine: {
                      show: false
                  },
                  axisTick: {
                      show: false
                  },
                  splitLine: {
                      show: false
                  }
  
              }
          ],
          yAxis:{
                  type: 'category',
                  data: ['0','1','2','3','4','5'],
                  axisLabel: {
                      color:"rgba(255,255,255,1)",
                      formatter: function (value) {
                              return '{' + value + '| }{value|'+array[value]+'}';
                      },
                      rich: {
                          value: {
                              align: 'center',
                              padding:[0,0,0,-5],
                              fontSize:12
                          },
                          0: {
                              height: 30,
                              align: 'center',
                              backgroundColor: {
                                  image: img['0']
                              }
                          },
                          1: {
                              height: 30,
                              align: 'center',
                              backgroundColor: {
                                  image: img['1']
                              }
                          },
                          2: {
                              height: 30,
                              align: 'center',
                              backgroundColor: {
                                  image: img['2']
                              }
                          },
                          3: {
                              height: 30,
                              align: 'center',
                              backgroundColor: {
                                  image: img['3']
                              }
                          },
                          4: {
                              height: 30,
                              align: 'center',
                              backgroundColor: {
                                  image: img['4']
                              }
                          },
                          5: {
                              height: 30,
                              align: 'center',
                              backgroundColor: {
                                  image: img['5']
                              }
                          }
                      }
                  },
                  axisTick: {
                      show: false
                  },
                  axisLine: {
                    show: false
                  },
                  zlevel: 99
              },
          series: [{
              type: 'bar',
              barWidth: '20%',
              barCategoryGap: '60%',
              itemStyle: {
                  normal: {
                      barBorderRadius:[10, 10, 10, 10],
                      color: new echarts.graphic.LinearGradient(
                              0, 0, 1, 0,
                              [
                                  {offset: 0, color: '#048BFE'},
                                  {offset: 1, color: '#00EEF1'}
                              ]
                      ),
                      label: {
                          show: true,
                          position: 'right',
                          textStyle: {
                              color: '#fff',
                              fontSize: '14'
                          }
                      }
                  }
              },
             // data: [99,85,74,63,66,61]
             data:data
          }]
      };
  
  
  
      option.series[0].data.reverse();
      myChart.setOption(option,true);

        
        
      });

}

//显示载客状态
function showCarryPassengerStatus(divId,startTimeId,endTimeId){

    var timeStart=document.getElementById(startTimeId).value;
    var timeEnd=document.getElementById(endTimeId).value;
    
    var dom = document.getElementById(divId);
    var myChart = echarts.init(dom);

    url="type=carryPassengerStatus"+"&startTime="+timeStart+"&endTime="+timeEnd;
    $.get("http://127.0.0.1:3000/?"+url,function(data){
        data=JSON.parse(data);   //data[0]空车数量，data[1]载客数量

        var axis= [{
            "name": "空车",
            "value": data[1],
            
        }, {
            "name": "载客",
            "value": data[0],
            
        }];

        var count=0;

        option = {
        backgroundColor:'#0c0f28',
        title:{
            x: 'center',
            y: '40%',
            text:'车辆总数',
            subtext:data[0]+data[1],
            itemGap:4,
            textStyle:{
                color:'#fff',
                fontSize:'13',
                fontWeight:'500'
            },
            subtextStyle: {
                color: '#f1af21',// 副标题文字颜色
                fontSize:'16',
                fontWeight:'bolder'
            }
        },

        legend: {
            top: "0",
            right:'0',
            textStyle: {
                color: '#fff',
                fontSize: 13,
                fontFamily:'宋体'
            },
            itemWidth:15,  //图例标记的图形宽度
            itemHeight:3, //图例标记的图形高度
        
            data: axis,
        },
        series: [
            // 主要展示层的
            {
                radius: ['54%', '60%'],
                center: ['50%', '50%'],
                selectedMode: 'single',
                selectedOffset: 5,
                type: 'pie',
                label: {
                    normal: {
                        show: false
                    },
                    emphasis: {
                        show: false
                    },
                    textStyle: {
                        color: '#fff',
                        fontSize: 14
                    }
                },
                labelLine: {
                    normal: {
                        show: false
                    },
                    emphasis: {
                        show: false
                    }
                },
                animation: false,
                data: [{
                    "name": "空车",
                    "value": data[1],
                    itemStyle: {
                        normal: {
                            color: '#4061d0'
                        }
                    }
                }, {
                    "name": "载客",
                    "value": data[0],
                    itemStyle: {
                        normal: {
                            color: '#a41d6e'
                        }
                    }
                }]
            },
            {
                radius: ['90%', '20%'],
                center: ['50%', '50%'],
                type: 'pie',
                color: ['#3c13a8', '#4d59e6'],
                selectedMode: 'single',
                selectedOffset: 5,
                label: {
                    normal: {
                        show: true,
                        textStyle: {
                            color: '#fff',
                            fontSize: 14
                        },
                        formatter: "{b} : {c} /{d}%"
                    },
                    emphasis: {
                        show: true
                    },
                    
                },
                labelLine: {
                    normal: {
                        show: true
                    },
                    emphasis: {
                        show: true
                    }
                },
                animation: false,
                data: [{
                    "name": "空车",
                    "value": data[1],
                    itemStyle: {
                        normal: {
                            color: 'rgba(76,118,255,.06)'
                        }
                    }
                }, {
                    "name": "载客",
                    "value": data[0],
                    itemStyle: {
                        normal: {
                            color: 'rgba(76,118,255,.06)'
                        }
                    }
                }]
            },
            
            // 边框的设置
            
            // 中心的圆圈
            {
                radius: ['38%', '54%'],
                center: ['50%', '50%'],
                type: 'pie',
                selectedMode: 'single',
                selectedOffset: 5,
                label: {
                    normal: {
                        show: false
                    },
                    emphasis: {
                        show: false
                    },
                    textStyle: {
                        color: '#fff',
                        fontSize: 14
                    }
                },
                labelLine: {
                    normal: {
                        show: false
                    },
                    emphasis: {
                        show: false
                    }
                },
                data: [{
                    "name": "空车",
                    "value": data[1],
                    itemStyle: {
                        normal: {
                            color: 'rgba(64,97,208,.5)'
                        }
                    }
                }, {
                    "name": "载客",
                    "value": data[0],
                    itemStyle: {
                        normal: {
                            color: 'rgba(164,29,110,.5)'
                        }
                    }
                }],
                animation: false
            },
        ]
        };

        function eConsole() {
        var r = count % axis.length;
        option.series[0].axis[r].selected = false;
        option.series[1].axis[r].selected = false;
        option.series[2].axis[r].selected = false;
        count++;
        var s = count % axis.length;
        option.series[0].axis[s].selected = true;
        option.series[1].axis[s].selected = true;
        option.series[2].axis[s].selected = true;

        //myChart.setOption(option, true);
        }
        myChart.setOption(option, true);
        myChart.on("click", eConsole);

    });

}


function showSpeedField(divId,startTimeId,endTimeId,inPutId){
	var timeStart=document.getElementById(startTimeId).value;
    var timeEnd=document.getElementById(endTimeId).value;
	var num=document.getElementById(inPutId).value;
	
	var dom = document.getElementById(divId);
    var myChart = echarts.init(dom);
	
	 url="type=showSpeedStatus"+"&startTime="+timeStart+"&endTime="+timeEnd+"&num="+num;
    $.get("http://127.0.0.1:3000/?"+url,function(data){
        data=JSON.parse(data);   //data[0]空车数量，data[1]载客数量
		var data1=[];
		var data2=[];
		var data3=[];
		for(var i=0;i<data.length;i+=2){
			var array=data[i].split(" ");
			data1.push(array[1]);//时间	
			data2.push(data[i+1]);//速度
			data3.push([array[1],data[i+1]]);
		}
		var schema = [
    {name: 'date', index: 0, text: '日'}, 
    {name: 'date', index: 1, text: '时间'},
    {name: 'date', index: 2, text: '速度'},
  
    
];
		 var option = {
          
           
            legend: {
                data: ['速度']
            },
            xAxis: {
				name: '时间',
                data: data1
            },
            yAxis: {
				name: '速度'
			},
			 tooltip: {
				  padding: 10,
        backgroundColor: '#222',
        borderColor: '#777',
        borderWidth: 1,
		formatter: function(value){
			return '时间: '+value.value[0]+'<br>'+'速度: '+value.value[1]+'km/h';
		}
            
		
				
			},
            series: [{
                name: '速度',
                type: 'line',
                data: data3
            }]
        };

        // 使用刚指定的配置项和数据显示图表。
        myChart.setOption(option);
	}
	);
}

function GetRandomNum(Min, Max) {
    var Range = Max - Min;
    var Rand = Math.random();
    return(Min + Math.round(Rand * Range));
}


	
function showHeatMap(divId,startTimeId,endTimeId){
	var timeStart=document.getElementById(startTimeId).value;
    var timeEnd=document.getElementById(endTimeId).value;
	var dom = document.getElementById(divId);
    var myChart = echarts.init(dom);
	url="type=showHeatMap"+"&startTime="+timeStart+"&endTime="+timeEnd;
    $.get("http://127.0.0.1:3000/?"+url,function(data){
		data=JSON.parse(data);
        var points=[];	
        var maxlength=0;
		var minlength=Number.POSITIVE_INFINITY;
        var length=0;
        var center=[];
        var point2=[0,0];		
        var class1=[];
        var class2=[];
        var class3=[];
        var classn=[class1,class2,class3];		
        var numi=0;		
		for(var i=0;i<data.length;i+=2){
			points.push([data[i],data[i+1]]);		
		}
		var num1=GetRandomNum(0,points.length-1);
		center.push([points[num1][0],points[num1][1]]);
		for(var i=1;i<points.length;i++){
		   length=Math.sqrt(Math.pow(points[i][0]-points[0][0],2)+Math.pow(points[i][1]-points[0][1],2));
		   if(length>maxlength){
			   maxlength=length;
			   point2[0]=points[i][0];
			   point2[1]=points[i][1];
		   }
		}//找第二个点
		center.push([point2[0],point2[1]]);
		maxlength=0;
		for(var i=0;i<points.length;i++){
			minlength=Number.POSITIVE_INFINITY;
			for(var j=0;j<2;j++){
				length=Math.sqrt(Math.pow(points[i][0]-center[j][0],2)+Math.pow(points[i][1]-center[j][1],2));
				if(length<minlength)minlength=length;
			}
			if(minlength>maxlength){
		    maxlength=minlength;
			numi=i;
			}
		}//找第三个聚类中心
		center.push([points[numi][0],points[numi][1]]);
		
		for(var i=0;i<points.length;i++){
			minlength=Number.POSITIVE_INFINITY;
			for(var j=0;j<3;j++){
				length=Math.sqrt(Math.pow(points[i][0]-center[j][0],2)+Math.pow(points[i][1]-center[j][1],2));
				if(length<minlength){
					minlength=length;
					numi=j;//记录当前点属于哪个类
				}
			}//三类循环
			
		    classn[numi].push(new BMap.Point(points[i][0],points[i][1],100));
			
		}
		var bmap = myChart.getModel().getComponent('bmap').getBMap();
       // myChart.clear();
		var heatmapOverlay = new BMapLib.HeatmapOverlay({"radius":20});
	    bmap.addOverlay(heatmapOverlay);
	    heatmapOverlay.setDataSet({data:class1,max:100});
		
		
	}
	);
}

